...
impl FieldAccessors {
            ...
            match &field.attrs.remote {
                Some(wrapper_ty) => {
                    quote! {
                        ...
                        unsafe { ::std::mem::transmute_copy::<_, #wrapper_ty>(&#accessor) }
                    }
                }
                None => accessor,
            }
        ...
        let fields_ref = Self::get_fields(reflect_struct, |field, accessor| {
            match &field.attrs.remote {
                Some(wrapper_ty) => {
                    quote! {
                        // SAFE: The wrapper type should be repr(transparent) over the remote type
                        unsafe { ::std::mem::transmute::<&_, &#wrapper_ty>(&#accessor) }
                    }
                }
                None => quote!(& #accessor),
            }
        ...
        let fields_mut = Self::get_fields(reflect_struct, |field, accessor| {
            match &field.attrs.remote {
                Some(wrapper_ty) => {
                    quote! {
                        // SAFE: The wrapper type should be repr(transparent) over the remote type
                        unsafe { ::std::mem::transmute::<&mut _, &mut #wrapper_ty>(&mut #accessor) }
                    }
                }
        ...
        F: FnMut(&StructField, proc_macro2::TokenStream) -> proc_macro2::TokenStream,
    {
        ...
        reflect_struct
            .active_fields()
            .map(|field| {
                let member = utility::ident_or_index(field.data.ident.as_ref(), field.index);
